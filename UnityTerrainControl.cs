...
public class UnityTerrainControl : MonoBehaviour
{
    //private Camera cam;
    ...
    private HashSet<int> _renderedBlocks = new HashSet<int>();
    private byte[] _mappingAlpha;
    private Color32[] colors;

    public UnityTerrainControl()
    {
    ...
    private void CreateTerrainNormal()
    {
        ...
        for (int z = 0; z < Constants.TERRAIN_SIZE; z++)
        {
            for (int x = 0; x < Constants.TERRAIN_SIZE; x++)
            {
                ...
                Vector3 v1 = new Vector3((x + 1) * Constants.TERRAIN_SCALE, z * Constants.TERRAIN_SCALE, _backTerrainHeight[GetTerrainIndexRepeat(x + 1, z)].b);
                Vector3 v2 = new Vector3((x + 1) * Constants.TERRAIN_SCALE, (z + 1) * Constants.TERRAIN_SCALE, _backTerrainHeight[GetTerrainIndexRepeat(x + 1, z + 1)].b);
                Vector3 v3 = new Vector3(x * Constants.TERRAIN_SCALE, (z + 1) * Constants.TERRAIN_SCALE, _backTerrainHeight[GetTerrainIndexRepeat(x, z + 1)].b);
                Vector3 v4 = new Vector3(x * Constants.TERRAIN_SCALE, z * Constants.TERRAIN_SCALE, _backTerrainHeight[GetTerrainIndexRepeat(x, z)].b);
    ...
    private void CreateTerrainLight()
    {
        ...
        Debug.Log("✅ _backTerrainLight has been initialized!");
    ...
    {
        int hash = block.Xi * 10000 + block.Zi;
        if (_renderedBlocks.Contains(hash))
            return true;

        _renderedBlocks.Add(hash);
        return false;
    }
    ...
    private void RenderTerrainTile(float xf, float zf, int xi, int zi, float lodf, int lodi)
    {
        ...
        var (_tempTerrainVertices, _tempTerrainTriangles) = PrepareTileVertices(xi, zi, xf, zf, idx1, idx2, idx3, idx4, lodf);

        if (isOpaque)
        {
            int number = 1;
            RenderTexture(_mapping.Layer2[idx1], _tempTerrainVertices, _tempTerrainTriangles, xf, zf, lodScale, number);
        }
        else
        {
            int number = 2;
            RenderTexture(_mapping.Layer1[idx1], _tempTerrainVertices, _tempTerrainTriangles, xf, zf, lodScale, number);
        }
    ...
    private (Vector3[], int[]) PrepareTileVertices(int xi, int zi, float xf, float zf, int idx1, int idx2, int idx3, int idx4, float lodf)
    {
        ...
        float sy = zf * Constants.TERRAIN_SCALE;
        float scaledSize = Constants.TERRAIN_SCALE * lodf;

        Vector3[] _tempTerrainVertices = new Vector3[4]
        {
            new Vector3(sx, terrainHeight1, sy),
            new Vector3(sx + scaledSize, terrainHeight2, sy),
            new Vector3(sx + scaledSize, terrainHeight3, sy + scaledSize),
            new Vector3(sx, terrainHeight4, sy + scaledSize)
        };
    ...
    private void RenderTexture(int textureIndex, Vector3[] _tempTerrainVertices, int[] _tempTerrainTriangles, float xf, float zf, float lodScale = 1.0f, int number = 0)
    {
        ...
        if(number == 1 || number == 2)
        {
            ...
            Material material = new Material(Shader.Find("Custom/SimpleTextureShader"));
            material.mainTexture = _textures[textureIndex];

            Mesh mesh = new Mesh();
            mesh.vertices = _tempTerrainVertices;
            mesh.triangles = _tempTerrainTriangles;
        ...
        if(number == 3)
        {
            ...
            Material alphaMaterial = new Material(Shader.Find("Custom/SimpleTextureAlphaShader"));
            alphaMaterial.mainTexture = _textures[textureIndex];
            alphaMaterial.color = new Color(1, 1, 1, 0.5f);
