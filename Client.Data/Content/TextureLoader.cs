...
namespace Client.Main.Content
{
    public class TextureLoader
    {
        ...
        private async Task<TextureData> InternalPrepare(string normalizedPath)
        {
            ...
            {
                //Debug.LogError($"Failed to load asset {normalizedPath}: {e.Message}");
                return null;
            }
        }
        ...
        private string FindTexturePath(string dataPath, string ext)
        {
            ...
            if (!File.Exists(fullPath))
            {
                //Debug.LogWarning($"Texture file not found: {fullPath}");
                return null;
            }
        ...
        public Texture2D GetTexture2D(string path)
        {
            ...
            var texture = new Texture2D((int)textureData.Info.Width, (int)textureData.Info.Height, TextureFormat.RGBA32, false);
            ...
            {
                //Debug.LogWarning($"Unsupported texture components: {components} for texture {path}");
                return null;
            }
